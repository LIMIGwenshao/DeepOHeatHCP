...
class Cuboid(object):
    ...
    def __init__(self,
        ...
        self.tensor, self.conductivity, self.beta, self.q = None, None, None, None #beta类似于sensor，q类似于power map
    ...
    def sample(self, srf, dim=3, mesh=False): #cuboid类中用于获取数据的函数。用于训练阶段，生成完整的训练数据
        if not mesh:
            ...
            self.beta, self.tensor, self.boundaries_set, self.conductivity = (
    ...
    def sample_grid_points(self, dim=3):  # 用于评估阶段，只生成评估所需的基本数据
        self.beta, self.tensor = sample_eval_data_single_domain(
            domain=self.domain_step, dim=dim
        )  # return sensors, eval_coords # sensor一维数组，每个元素对应一个点的温度值；tensor是坐标
        self.q, self.conductivity = np.array([]), np.array([])
...
def fetch_data(geometry, mode="train", *args):#可以保留
