...
class Cuboid(object):
    ...
    def __init__(self, domain, starting_idx=0, parent=None, parent_boundary=None):
        ...
        self.beta_sampling_list = [] #这里是特有的变量初始化
        ...
        self.power_points_set_dict = {}#这里是特有的变量初始化
        self.boundaries_set = {key: OrderedSet() for key in self.boundaries_list}
        self.interface_set = OrderedSet()
    ...
    def sample(self, sample_domain=True):
        ...
        if sample_domain:
            self.domain_step = self.sample_domain() #默认的采样模式是任意值采样
    ...
    def sample_domain(self, sample_mode="random"):#封装一个包含self.beta数据的的domain,beta是对某个参数的采样
        ...
        for boundary_name, boundary_variable_params in self.domain["parameterized"]["param_space" ].items():
    ...
    def find_inside_set(self, whole_set, power_set, boundaries_set):
        return whole_set - OrderedSet().union(*boundaries_set.values()) - power_set #多减了一个power set

    def update_set(self):#这一步才进行了连接处点的剔除
    ...
    def find_boundries_set(self, domain, tensor, boundaries_set):
        # node以及adjacent_boundary_name用于发现相邻边界的范围，boundary_set是自己的
        ...
        def find_node_boundary_set(node, adjacent_boundary_name, boundary_set):
            ...
            #将全局索引转化为局部索引，减去该几何的起始索引
            boundary_array = np.array(list(boundary_set))  # 转为 numpy 数组
            adjusted_array = boundary_array - self.starting_idx  # 批量减法
            boundary_set_minus_starting = OrderedSet(adjusted_array.tolist())  # 转回 OrderedSet
            # print(adjacent_boundary_name)
            # print(boundary_set_minus_starting)
            # print(boundary_set)
            # print(tensor.shape)
            ...
            # print(ends)
            # print("=====================")
            return find_set_by_range_in_subset(tensor, boundary_set_minus_starting, starts, ends)
        ...
        def find_single_boundary_set(boundary_name):
            ...
            boundary_name_idx = self.boundaries_list.index(boundary_name) #返回boundary.name出现的位置
            ...
            boundary_set = boundaries_set[boundary_name] #此处的边界索引是该节点的，不是指向节点的
            adj_boundary_set = OrderedSet() #此处后面得到的是在此节点的边界索引中，与相邻子域接触的点的索引

            #这里一直有问题，也是导致不能进行多层堆叠的地方
            if not self.is_root() and self.parent_boundary == adjacent_boundary_name:
                ...
                #print("adj_boundary_set", adj_boundary_set)
            ...
            #这里先进行判断，避免有值的情况下被覆盖
            if not self.interface_set: #这里为什么会循环6次，后续需要细看
                self.interface_set = adj_boundary_set
            #print(f"self.interface_set = {self.interface_set}")
...
def fetch_data(geometry, mode="train", *args): #*args是sample_domain=True
    ...
    def fetch_single_node(node):
        ...
        beta_list.append(node.beta) #等价于sensors
