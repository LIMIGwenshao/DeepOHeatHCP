...
class CuboidGeometryDataIO(Dataset):
    ...
    def __init__(self, domains_list, global_params, beta_as_input=False): #默认false，在实验中为false
        super().__init__()
        ...
        self.beta_as_input = beta_as_input #false
        self.mode = "train"
        self.args = []
    ...
    def train(self, sample_domain=True):
        ...
        coords, conductivity, beta = next(iter(self))#调用getitem里的fetch_data函数，最后调用sample函数采样
        beta = beta.repeat(coords.shape[0], 1)

        if self.beta_as_input: #false，默认不调用
