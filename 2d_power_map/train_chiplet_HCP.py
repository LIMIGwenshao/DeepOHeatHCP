...
os.chdir(dir_path)
sys.path.append("../../")
from src import dataio_deeponet, training_deeponet_chipletpartiallyhcp, loss_fun_deeponet, modules
    ...
    power=dict(
        ...
        power_map=dict(
            power_0=dict(
                type="surface_power",
                surface="bottom",
                location=dict(starts=(10, 0, 0), ends=(20, 10, 0)),
...
val_func = training_deeponet_chipletpartiallyhcp.val_fn_init(False) #只用一半的左边，是切片（可能用于评估）
...
tic = time.time()
training_deeponet_chipletpartiallyhcp.train_mesh(
    model=model,
    dataset=dataset,
    epochs=epochs,
    lr=lr,
