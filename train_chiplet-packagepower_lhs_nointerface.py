...
os.chdir(dir_path)
sys.path.append("../../")
from src import dataio, training_hcp_LHS_nointerface, loss_fun, modules
domain_0 = dict(
    domain_name=0,
    geometry=dict(
        starts=[0.0, 0.0, 0.0],
...
domain_1 = dict(
    ...
    left=dict(bc=True, type="adiabatics", params=dict(dim=0, weight=1)),
    right=dict(bc=True, type="adiabatics", params=dict(dim=0, weight=1)),
    bottom=dict(bc=True, type="adiabatics", params=dict(dim=2, weight=0.01)),
...
val_func = training_hcp_LHS_nointerface.val_fn_init(False)

root_path = "./log"
experiment_name = "10000-chiplet-lhs-nointerface"
...
tic = time.time()
training_hcp_LHS_nointerface.train(
    model=model,
    dataset=dataset,
    epochs=epochs,
    lr=lr,
